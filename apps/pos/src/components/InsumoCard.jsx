  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <h3 className="font-semibold text-[#42241A] text-lg leading-tight">{insumo.nombre}</h3>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          ...
          <p className={`text-2xl font-bold ${isCritical ? 'text-red-600' : 'text-[#42241A]'}`}>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          Costo:{' '}
          <span className="font-medium text-[#42241A]">
            ${Number(insumo.costo_unitario).toFixed(2)}/{insumo.unidad}
          </span>
        </p>
        <button
          onClick={() => onRestock(insumo)}
          className="flex items-center gap-2 bg-[#4E5B3D] hover:bg-[#3E4A30] text-[#FAFAF7] font-semibold px-4 py-2 rounded-xl transition-colors min-h-[44px]"
        >
          <RotateCcw size={16} />
          Restock
        </button>
